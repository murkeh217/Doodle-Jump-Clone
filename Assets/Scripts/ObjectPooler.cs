...
public class ObjectPooler : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (place)
		{ //Runs only when place is set True in GetPlatform() so that values of x and y don't keep on updating every frame else*
	...
	public void GetPlatform()
	{
		...
		*/
		...
		{
			cummulative += prob[i];
			print(cummulative + "Cumulative");
			if (ind < cummulative)
			{
				for (int j = 0; j < pooledObjects[i].Count; j++)
				{   //This for loop is used to find the gameobject which is not active in the hierarchy
